              ...
              {props.images.map(item=> {
                return<div className="list-grid" key={item.flight_number}>
                         <div className="image-card"> 
                              <img className="image" src={item.links.mission_patch_small}  />
                         </div>
                        <div className="image-content">
                          <div className="title">{item.mission_name} # {item.flight_number}</div> 
                          <div className="missionId">Mission Ids : <div style={{textAlign:"center", fontWeight:"normal", color:"blue"}}> {item.mission_id.length > 0 ? `${item.mission_id}` : 'NA' } </div>  
